...
#if ART_USE_FUTEXES
  futexData_->state_ = 0;
  futexData_->exclusive_owner_ = 0;
  futexData_->num_contenders_ = 0;
#else
  CHECK_MUTEX_CALL(pthread_mutexattr_init,
      (&futexData_->attr_));
  CHECK_MUTEX_CALL(pthread_mutexattr_setpshared,
      (&futexData_->attr_, PTHREAD_PROCESS_SHARED));
  if(futexData_->recursive_) {
    CHECK_MUTEX_CALL(pthread_mutexattr_settype,
        (&futexData_->attr_, PTHREAD_MUTEX_RECURSIVE));
  }
  CHECK_MUTEX_CALL(pthread_mutex_init, (&futexData_->mutex_, &futexData_->attr_));
  //
#endif
}
...
InterProcessMutex::~InterProcessMutex(){
  ...
  if(futexData_->state_ != 0) {
    ...
    LOG(shutting_down ? WARNING : FATAL) << "destroying mutex with owner: " <<
        futexData_->exclusive_owner_;
  } else {
    CHECK_EQ(futexData_->exclusive_owner_, 0U)  <<
        "unexpectedly found an owner on unlocked mutex " << name_;
    CHECK_EQ(futexData_->num_contenders_, 0) <<
  ...
  int rc, rc_attr = 0;
  rc = pthread_mutex_destroy(&futexData_->mutex_);
  if(rc == 0)
    rc_attr = pthread_mutexattr_destroy(&futexData_->attr_);
  if (rc != 0 || rc_attr != 0) {
    int flag = 0;
    if(rc != 0)
      errno = rc;
    ...
    PLOG(shutting_down ? WARNING : FATAL) <<
        ...
        flag ? "destroying the attribute" : "destroying the mutex";
  }
#endif
}//~InterProcessMutex()


void InterProcessMutex::RegisterAsInterProcessLocked(Thread* self) {
...
void InterProcessMutex::RegisterAsInterProcessUnlocked(Thread* self) {
  if (UNLIKELY(self == NULL)) {
    CheckUnattachedThread(level_);
    return;
  }
  if (level_ != kMonitorLock) {
    if (kDebugLocking && !gAborting) {
      CHECK(self->GetHeldMutex(level_) == this) <<
          "Unlocking on unacquired mutex: " << name_;
    }
    self->SetHeldMutex(level_, NULL);
  }
}

void InterProcessMutex::ExclusiveLock(Thread* self) {
  DCHECK(self == NULL || self == Thread::Current());
  if (kDebugLocking && !futexData_->recursive_) {
    AssertNotHeld(self);
  }
  if (!futexData_->recursive_ || !IsExclusiveHeld(self)) {
#if ART_USE_FUTEXES
    bool done = false;
    do {
      ...
      if (cur_state == 0) {
        ...
        //TODO: Fizo, do not track contention here..ScopedContentionRecorder scr(this, SafeGetTid(self), GetExclusiveOwnerTid());
        android_atomic_inc(&futexData_->num_contenders_);
        if (futex(&futexData_->state_, FUTEX_WAIT, 1, NULL, NULL, 0) != 0) {
        ...
        android_atomic_dec(&futexData_->num_contenders_);
      }
    } while (!done);
    DCHECK_EQ(futexData_->state_, 1);
    futexData_->exclusive_owner_ = SafeGetTid(self);
#else
    CHECK_MUTEX_CALL(pthread_mutex_lock, (&futexData_->mutex_));
#endif
    RegisterAsInterProcessLocked(self);
  }
  futexData_->recursion_count_++;
  if (kDebugLocking) {
    CHECK(futexData_->recursion_count_ == 1 || futexData_->recursive_) <<
        "Unexpected recursion count on mutex: "
        << name_ << " " << futexData_->recursion_count_;
    AssertHeld(self);
  }
}
...
bool InterProcessMutex::ExclusiveTryLock(Thread* self) {
  ...
  if (!futexData_->recursive_ || !IsExclusiveHeld(self)) {
    ...
    futexData_->exclusive_owner_ = SafeGetTid(self);
#else
    int result = pthread_mutex_trylock(&futexData_->mutex_);
    if (result == EBUSY) {
      return false;
    }
    if (result != 0) {
...
void InterProcessMutex::ExclusiveUnlock(Thread* self) {
  DCHECK(self == NULL || self == Thread::Current());
  AssertHeld(self);
  futexData_->recursion_count_--;
  if (!futexData_->recursive_ || futexData_->recursion_count_ == 0) {
    if (kDebugLocking) {
      CHECK(futexData_->recursion_count_ == 0 || futexData_->recursive_) <<
          "Unexpected recursion count on mutex: "
          << name_ << " " << futexData_->recursion_count_;
    }
    RegisterAsInterProcessUnlocked(self);
#if ART_USE_FUTEXES
  bool done = false;
  do {
    int32_t cur_state = futexData_->state_;
    if (cur_state == 1) {
      ...
      futexData_->exclusive_owner_ = 0;
      // Change state to 0.
      done = android_atomic_release_cas(cur_state, 0, &futexData_->state_) == 0;
      if (done) {  // Spurious fail?
        // Wake a contender
        if (futexData_->num_contenders_ > 0) {
          futex(&futexData_->state_, FUTEX_WAKE, 1, NULL, NULL, 0);
        }
      }
    } else {
...
#else
    CHECK_MUTEX_CALL(pthread_mutex_unlock, (&futexData_->mutex_));
#endif
  }
}


uint64_t InterProcessMutex::GetExclusiveOwnerTid() const {
#if ART_USE_FUTEXES
  return futexData_->exclusive_owner_;
#elif defined(__BIONIC__)
  return static_cast<uint64_t>((futexData_->mutex_.value >> 16) & 0xffff);
#elif defined(__GLIBC__)
  return reinterpret_cast<const glibc_pthread_mutex_t*>(&futexData_->mutex_)->owner;
#elif defined(__APPLE__)
  const darwin_pthread_mutex_t* dpmutex = reinterpret_cast<const darwin_pthread_mutex_t*>(&futexData_->mutex_);
...
void InterProcessMutex::Dump(std::ostream& os) const {
  os << (futexData_->recursive_ ? "recursive " : "non-recursive ")
      << name_
      << " level=" << static_cast<int>(level_)
      << " rec=" << futexData_->recursion_count_
      << " owner=" << GetExclusiveOwnerTid() << " ";
  //TODO:: fizo handle contention later between processes...DumpContention(os);
}
