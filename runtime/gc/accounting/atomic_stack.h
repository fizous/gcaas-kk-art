...
class StructuredAtomicStack {
 public:
  ...
  virtual void Reset() {
    ...
    LOG(ERROR) << "front_index address = " << StringPrintf("%p", static_cast<uint32_t>(&(stack_data_->front_index_)));
    int32_t old_back;
    do {
      old_back = stack_data_->back_index_;
    } while (android_atomic_cas(old_back, 0, &(stack_data_->back_index_)) != 0);
    LOG(ERROR) << "back_index address = " << StringPrintf("%p", static_cast<uint32_t>(&(stack_data_->back_index_)));
