...
class StructuredAtomicStack {
 public:
  ...
  static StructuredAtomicStack* Create(const std::string& name, size_t capacity, bool shareMem) {
  ...
  void Reset() {
    ...
    if(stack_data_->is_shared_) {
      posix_madvise(stack_data_->begin_, sizeof(T) * stack_data_->capacity_,
          POSIX_MADV_DONTNEED);
      //memset(stack_data_->begin_, 0, sizeof(T) * stack_data_->capacity_);
    } else {
