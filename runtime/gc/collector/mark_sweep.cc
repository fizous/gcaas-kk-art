    ...
    if (klass->IsObjectArrayClass()) {
      ...
      for (size_t i = 0; i < length; ++i) {
        ...
        LOG(ERROR) << "client; arr; " << object << "; length; " << length <<
            ...
            "; marked; " <<
            StringPrintf("%s", ((_element_i == nullptr) ? "XX" : (ismrk ? "true" : "false")));
      }
...
bool MarkSweep::IsMarkedNoLocks(const mirror::Object* object,
  ...
  accounting::BASE_HEAP_BITMAP_T* heap_beetmap = (accounting::BASE_HEAP_BITMAP_T*)_heap_beetmap;
  ...
  for (const auto& space : GetHeap()->GetContinuousSpaces()) {
    if (space->IsDlMallocSpace()) {
      ...
      if(space->HasBitmapsBound()) {
        ...
        GetHeap()->GetMarkBitmap()->ReplaceBitmap(new_bitmap,
                                              alloc_space->mark_bitmap_.get());
      }
